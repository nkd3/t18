# PS 5.1-compatible console tracer — C:\T18\ops\trace_console.ps1
$Log = 'C:\T18\logs\noflash_watch.log'
...
$null = Register-WmiEvent -Query $Query -SourceIdentifier TraceProcStart

try {
  $stopAt = (Get-Date).AddMinutes(20)
  while ((Get-Date) -lt $stopAt) {
    $evt = Wait-Event -SourceIdentifier TraceProcStart -Timeout 1
    if ($null -ne $evt) {
      $p = $evt.SourceEventArgs.NewEvent

      $name = ""; if ($p.ProcessName) { $name = [string]$p.ProcessName }
      $nameLower = $name.ToLower()

      if ($Targets -contains $nameLower) {
        $cmd = ""; if ($p.CommandLine) { $cmd = ([string]$p.CommandLine -replace '\s+',' ').Trim() }

        $procId  = 0
        $parentId = 0
        if ($p.ProcessID)       { $procId  = [int]$p.ProcessID }
        if ($p.ParentProcessID) { $parentId = [int]$p.ParentProcessID }

        $parentInfo = "PPID=$parentId"
        try {
          $parent = Get-Process -Id $parentId -ErrorAction SilentlyContinue
          if ($parent) { $parentInfo = "$($parent.ProcessName).exe (PPID=$parentId)" }
        } catch {}

        Log-Line "PROC START name=$name PID=$procId $parentInfo CMD=$cmd"
      }
